...
import "./Medicines.css"; // Asegúrate de crear este archivo CSS
...
const Medicamentos = () => {
  ...
  const medicamentos = [
    ...
    },
    {
      ...
      descripcion: "Descripción del Medicamento B",
      ...
      <section className="mt-12 max-w-screen-lg mx-auto px-4 md:px-8">
        ...
        <ul className="mt-12 space-y-6">
          {medicamentos.map((item, idx) => (
            <li
              key={idx}
              className="p-5 bg-white border-gray-600 border-[1px] rounded-md shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300"
            >
              <a href={item.href}>
                <div>
        ...
        <ul className="medicamentos-list">
          {medicamentos.map((medicamento, index) => (
            <li key={index} className="medicamento-item">
              <h3 class="font-bold ">{medicamento.nombre}</h3>
              <p>Descripción: {medicamento.descripcion}</p>
              <p>Fecha de Caducidad: {medicamento.fechaCaducidad}</p>
            </li>
          ))}
        </ul>
      </div>
